...
class GUI:
    ...
    def goal_reminder(self):
        ...
        self.frames[BudgetFrame].children[goal_frame.goalReminder.winfo_name()].config(text = f"Goal: {goal}")
        self.show_frame(MenuFrame)

        # links the data to the month and stores it
    def submit_month(self):

        month_frame = self.frames[MenuFrame]
    ...
    def to_recommendations(self):
        ...
        self.show_frame(RecFrame)

    def display_history(self, controller):

        history_frame = self.frames[HistoryFrame]
        history_frame.historyText.delete('1.0', tk.END)
        ...
        controller.show_frame(HistoryFrame)
...
class GoalFrame(tk.Frame):
    ...
    def __init__(self, master, controller):
        ...
        ttk.Label(master = self, text = "What is your budgeting goal?", font = ("Times New Roman", 20)).pack(pady = 5)
        # entry box for the goal
        self.goalEntry = ttk.Entry(master = self, font = ("Times New Roman", 14), width = 50)
...
class MenuFrame(tk.Frame):
    ...
    def __init__(self, master, controller):
        ...
        # recommendations
        ttk.Button(master = self, text = "Recommendations", command = controller.to_recommendations).pack(pady = 10)
